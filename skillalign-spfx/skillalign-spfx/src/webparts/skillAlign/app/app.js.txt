...
async function renderUserDashboard() {
    const topics = await LocalData.loadTopics();
    const assessments = await LocalData.getUserAssessments();
...
async function startRandomAssessment(topicName) {
    ...
    const questions = await LocalData.loadRandomQuestions(topicName, 30);
...
async function handleAddTopic(event) {
    ...
    const result = await LocalData.addTopic({
...
async function handleAddQuestion(event) {
    ...
    const result = await LocalData.addQuestion({
...
async function renderAdminDashboard() {
    const assessments = await LocalData.getAllAssessments();
    const users = await LocalData.getAllUsers();
    const topics = await LocalData.loadTopics();
...
async function submitAssessment() {
    ...
    const passed = score >= LOCAL_CONFIG.passingScore;
    const timeTaken = Math.round((Date.now() - state.assessmentStartTime) / 1000 / 60);
    
    const result = await LocalData.saveAssessment({
